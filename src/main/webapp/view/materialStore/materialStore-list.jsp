...
<body class="layui-anim layui-anim-up">
	<div class="x-nav">
		<span class="layui-breadcrumb"> <a>仓库管理</a> <a> <cite>原料出入库统计表</cite></a>
	...
	<script>
		$(function() {
			...
			$.ajax({
				url : 'materialController/getPage.ajax',
				type : 'POST',
				data : {
					sreach : sreach,
					sreach2 : sreach2,
		...
		function showpage(n) {
			...
			//显示页面内容
					...
					$.ajax({
						url : 'materialController/getinfoTable.ajax',
						type : 'POST',
						data : {
							n : n,
							sreach : sreach,
						...
						success : function(res) {
							...
							$.each(jtable,function(n, val) {
												$("#infotable").append(
																"<tr id='"+val.materialOutInId+"'><td>"+ val.warehouseName+"</td><td>"+val.materialName +"</td><td>"+val.materialInventoryAmount
																+"</td><td>"+val.materialInventoryValue+"</td><td>"+val.goodsType+"</td><td>"+val.time
																 + " </td></tr>");
											});
						},
						error : function() {
		...
		function doSearch(){
			var s=$("#search").val();
			var s2=$("#search2").val();
			window.location.href='<%=basePath%>materialController/sreach.do?sreach='+s+'&sreach2='+s2;
		}
