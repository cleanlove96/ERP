...
<body>
	...
	<xblock>
		...
		<button class="layui-btn"
			onclick="x_admin_show('添加用户','view/yearplan/yearplan-add.jsp',500,500)">
...
<script type="text/javascript">
	...
	$(function() {
		...
		getpage();
		showYeartablepage(pn);
	})
		/*用户-删除*/
		function member_del(obj, yearId) {
			layer.confirm('确认要删除吗？', function(id) {
				$.ajax({
					url : 'yearplanController/deleteCommodityById.ajax',
					type : 'POST',
					/*async:false, ajax同步*/
					data : {
						id:yearId
		...
		function doSearchCard(){
			...
			mycommodity=$("#selectSomeInfo").val()
			window.location.href = 'yearplanController/doSearchCard.do?searchCard='+ s+'&mycommodity='+ mycommodity;
		}
		/*改变商品名,查看相应的计划表信息*/
		...
		function showYeartablepage(n) {
			...
			$.ajax({
				...
				success : function(res) {
					...
					$("#yearplanTable").append("<tr><td>"+val.commodityId+"</td><td>"+val.yearNum+"</td><td>"+val.goodsSum+"</td><td>"+val.startTime+"</td><td>"+val.endTime+"</td><td>"+val.accountId+"</td><td>"+val.yearpanlVariety+"</td><td class='td-manage'> <a onclick=\"x_admin_show('修改计划表','yearplanController/selectByYearpanlTableId.do?yearpanlTableId="+val.yearpanlTableId+"',600,400)\" title='修改计划表' href='javascript:;'> <i class='layui-icon'>&#xe631;</i></a> <a title='删除' onclick=\"member_del(this,'"+val.yearpanlTableId+"')\"  href='javascript:;'> <i class='layui-icon'>&#xe640;</i> </a> </td> </tr>");
							});	
						},
				error : function() {
